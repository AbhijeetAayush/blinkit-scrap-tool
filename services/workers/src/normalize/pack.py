...
def packs_compatible(a_ml: int | None, b_ml: int | None, tolerance: float = 0.05) -> bool:
    if a_ml is None or b_ml is None or a_ml <= 0 or b_ml <= 0:
        return False
    return abs(a_ml - b_ml) / max(a_ml, b_ml) <= tolerance
